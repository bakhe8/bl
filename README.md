# نظام توليد خطابات تمديد الضمانات البنكية — إصدار 1.0.1
...
## ملاحظات البيئة
- التطبيق يعمل بلا متغيرات بيئة إلزامية.
- عند الحاجة لإعدادات مخصّصة (مثلاً `VITE_API_URL`) أنشئ ملف `.env.local` في الجذر. يجب أن تبدأ متغيرات الواجهة بـ `VITE_`.
- تجنّب وضع أسرار في المستودع العام؛ استخدم `.env.local` محلياً أو أسرار CI.
