...
لتخصيص نقاط النهاية والإعدادات القابلة للتغيير، استخدم ملفات البيئة الخاصة بـ Vite.

- نسخ المثال المحلي (موجودة بالعربية والإنجليزية في الجذر):

```powershell
Copy-Item .env.example.ar .env.local
notepad .env.local
```

- في حال استخدام متغيرات في الكود من جهة العميل، تأكد أنها تبدأ بـ `VITE_` (مثل `VITE_API_URL`).
- لا تضع مفاتيح سرية أو كلمات مرور في ملفات التي تُرحَّل إلى مستودعات Git. استخدم `.env.local` أو سرّ مخفي في CI/CD.

أضفت ملفات مثال ` .env.example` و ` .env.example.ar` إلى المشروع لتسهيل التشغيل المحلي.
