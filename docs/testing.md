...
- اختبار طباعة PDF في Chrome وEdge مع التحقق من RTL والخط والتباعد.
- اختبار تجاوز LocalStorage (قريب من الحد) وإظهار التحذير.
- اختبار mapping خاطئ (عمود واحد مع حقلين) وظهور تحذير أحمر وإجبار المستخدم على التصحيح.
- اختبار Fuzzy عند ثقة منخفضة (<0.7) للتأكد من الرفض التلقائي.
- اختبار رفض ملفات غير UTF-8 أو تظهر برموز "؟؟؟" مع رسالة واضحة.
- اختبار إنشاء variant جديد وإظهار تحذير، ثم تأكيده بعد ظهوره في 3 ملفات مختلفة.
- اختبار غياب زر التراجع: أي قرار variant يُحفظ فوراً ويظهر تحذير عدم التراجع.
- اختبار إنشاء مورد جديد: رسالة تأكيد صريحة قبل الإضافة، ولا تراجع بعدها.
