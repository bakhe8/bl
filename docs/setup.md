# الإعداد والتشغيل

## البيئة المقترحة
- نظام التشغيل: Windows (أو أي نظام يحوي متصفح حديث).
- المتصفح: Chrome أو Edge.
- الاتصال: غير مطلوب؛ التطوير يتم عبر خادم Vite محلي ثابت (5173).

## خطوات التشغيل المبدئية
1) استنسخ المستودع محلياً.
2) ثبّت التبعيات: `npm install`.
3) شغّل الخادم: `npm run dev` (أو `npm start` لاستعمال سكربت PowerShell الذي يحرر المنفذ 5173 ويشغّل Vite).
4) افتح المتصفح على `http://localhost:5173/`.
5) حمّل ملف Excel، حلّ الغموض، ثم اطبع بصيغة PDF من المتصفح.

### سكربت التشغيل (اختياري)
- `run-dev.ps1` في جذر المشروع:
  - يغلق أي عملية Vite متبقية.
  - يحرر المنفذ 5173 إن كان مشغولاً.
  - يشغّل `npm run dev` على منفذ ثابت 5173 (`strictPort: true`).

### ملاحظات تشغيل
- لا تستخدم Live Server أو python server؛ Vite فقط.
- إذا ظهر "Port 5173 is in use" شغّل `npm start` مجدداً (السكربت يغلق Vite القديم).
- لتحديث المنفذ غيّر `port` في `vite.config.js` و/أو سكربت `run-dev.ps1`.

## الاعتمادات
- تستخدم React/Vite مع SheetJS لقراءة ملفات Excel.
- التخزين في LocalStorage للقاموس، مع خيار التصدير/الاستيراد بصيغة JSON.

## متغيرات البيئة
- لا توجد متغيرات إلزامية.
- يمكن إضافة ملف `.env.local` للقيم الاختيارية (يجب أن تبدأ متغيرات الواجهة بـ `VITE_`).

## النسخ الاحتياطي والاستعادة
- وفّر خيار تصدير القاموس إلى ملف JSON دوريًا (خاصة عند العمل على أكثر من جهاز).
- للاستعادة، استورد ملف JSON لإعادة تدريب القاموس سريعاً.

## الدعم المستقبلي
- عند إضافة حزم/بناء حدّث ملف CI في `.github/workflows/ci.yml` وخيارات التخزين/النسخ الاحتياطي هنا.
